...
function createShader(gl: WebGL2RenderingContext, shaderCode: string) {
...
async function createShaderFromUrl(gl: WebGL2RenderingContext, url: string): Promise<WebGLProgram> {
    let shaderCode = ''
    await fetch(url)
        .then(response => response.text())
        .then(data => shaderCode += data)
    
    return createShader(gl, shaderCode)
}
